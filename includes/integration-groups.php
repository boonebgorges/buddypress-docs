	...
	function map_meta_caps( $caps, $cap, $user_id, $args ) {
		...
		if ( isset( $wp_post_types[$pt] ) && in_array( $cap, (array)$wp_post_types[$pt]->cap ) ) {
			...
			foreach( (array)$ugs as $ug ) {
				if ( $ug->is_admin ) {
					$user_groups[$ug->group_id] = 'admin';
				} else if ( $ug->is_mod ) {
					$user_groups[$ug->group_id] = 'mod';
				} else {
					$user_groups[$ug->group_id] = 'member';
				}
			}
		}
		...
		if ( !empty( $groups ) ) {		
			switch ( $cap ) {
				case 'read_bp_doc' :
					...
					// The issue of multiple groups really complicates things
					// For now, if *any* group is non-public, then you must be
					...
					switch ( $edit_setting ) {						
						...
						case 'admins-mods' :
							...
							foreach( $groups as $g_id ) {
								if ( !empty( $user_groups[$g_id] ) && in_array( $user_groups[$g_id], array( 'admin', 'mod' ) ) ) {
									$caps = array( $cap );
								}
							}
							break;
