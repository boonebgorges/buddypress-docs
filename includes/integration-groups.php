...
class BP_Docs_Groups_Integration {
	 ...
	 * @return array
	 */
	function activity_args( $args, $query ) {
		$group_id = ! empty( $_POST['associated_group_id'] ) ? intval( $_POST['associated_group_id'] ) : 0;

		if ( ! empty( $group_id ) ) {
			// update existing activity entry for created group docs
			if ( empty( $query->is_new_doc ) ) {
				// try to find the activity entry
				$activity = bp_activity_get( array(
					// this accounts for any group privacy setting
					'show_hidden' => 1,
					...
					'filter' => array(
						'object'       => 'groups',
						'action'       => 'bp_doc_created',
						'primary_id'   => $group_id,
						'secondary_id' => $query->doc_id
					)
				) );

				// we found it!
				// update the original 'bp_doc_created' activity entry
				if ( $activity['total'] > 0 ) {
					$args['id'] = $activity['activities'][0]->id;
				}
			}
